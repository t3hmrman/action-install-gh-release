...
async function run() {
    try {
        ...
        let asset = getReleaseUrl.data.assets.find(obj => {
            ...
            if (!nameIncluded) { core.info(`name [${assetName}] wasn't included in [${normalized}]`); }
            const osArchMatches = osArchRegex.test(normalized);
            if (!osArchMatches) { core.info("osArch didn't match"); }
            const osMatches = osRegex.test(normalized);
            if (!osMatches) { core.info("os didn't match"); }
            const vendorMatches = vendorRegex.test(normalized);
            if (!vendorMatches) { core.info("vendor didn't match"); }
            const libcMatches = libcRegex.test(normalized);
            if (!libcMatches) { core.info("libc calling didn't match"); }
            const extensionMatches = extensionRegex.test(normalized);
            if (!extensionMatches) { core.info("extenison didn't match"); }
